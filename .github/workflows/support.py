...
def init_db():
    ...
    c.execute('''CREATE TABLE IF NOT EXISTS stock_suggestions (
                    id INTEGER PRIMARY KEY AUTOINCREMENT,
                    stock TEXT,
                    avg_price REAL,
                    risk_level TEXT
                )''')
    # Add example stock data (adjust as needed)
    c.executemany('''INSERT OR IGNORE INTO stock_suggestions (stock, avg_price, risk_level)
                     VALUES (?, ?, ?)''', [
                    ("Stock A", 100, "low"),
                    ("Stock B", 200, "medium"),
                    ("Stock C", 300, "high")
...
def meraSunburst(df=None, height=None, width=None):
    ...
    fig.update(layout_showlegend=False)  # Hide the legend if you want to
    
    # Return the figure as a JSON object for rendering in the template
    return json.dumps(fig, cls=plotly.utils.PlotlyJSONEncoder)
def month_bar(df=None, height=None, width=None):
    # Ensure 'Amount(₹)' column is numeric (convert if needed)
    df['Amount(₹)'] = pd.to_numeric(df['Amount(₹)'], errors='coerce')  # This will convert non-numeric values to NaN
    
    # Check if 'Month' column is in text or numeric format
    if df['Month'].dtype == 'object':  # If 'Month' is a text column
        # Mapping text to month names
        month_map = {
            "January": 1, "February": 2, "March": 3, "April": 4, "May": 5, "June": 6,
            "July": 7, "August": 8, "September": 9, "October": 10, "November": 11, "December": 12
        }
        # Convert month names to month numbers (1-12)
        df['Month'] = df['Month'].apply(lambda x: month_map.get(x, x))  # Apply the mapping safely
    elif df['Month'].dtype == 'int64':  # If 'Month' is numeric
        month_names = ["January", "February", "March", "April", "May", "June", "July", "August", "September", "October",
                       "November", "December"]
        # Convert numeric month to name (1 -> January, 2 -> February, etc.)
        df['Month'] = df['Month'].apply(lambda x: month_names[x - 1] if 1 <= x <= 12 else x)

    # Group by 'Month' and 'Expense', then sum 'Amount(₹)'
    t = df.groupby(['Month', 'Expense']).sum(numeric_only=True).reset_index()[['Month', 'Expense', 'Amount(₹)']]

    # Create bar chart with Plotly
    fig = px.bar(t, x='Month', y='Amount(₹)', color='Expense', text_auto=True, height=height, width=width,
                 template='plotly_dark')

    # Update layout and appearance of the chart
    ...
    # Return the figure as a JSON object for rendering
    return json.dumps(fig, cls=plotly.utils.PlotlyJSONEncoder)